...
export class SourceDemo {
  ...
  findMessage(type: typeof Message | ((msg: Message) => boolean)) {
    const byType =
      type.prototype instanceof Message
  ...
  findPacket(type: typeof NetMessage | ((packet: NetMessage) => boolean)) {
    const byType =
      type.prototype instanceof NetMessage
        ? (packet: NetMessage) => packet instanceof type
        : (packet: NetMessage) =>
            (type as (msg: NetMessage) => boolean)(packet);
  ...
  readMessages(buf: SourceDemoBuffer) {
    ...
    const demoMessages = readSlot
      ? DemoMessages.NewEngine
      : DemoMessages.OldEngine;
  ...
  readPackets(netMessages?: (typeof NetMessage | undefined)[]) {
    netMessages =
      netMessages ??
      (this.isNewEngine()
        ? NetMessages.Portal2Engine
        : NetMessages.HalfLife2Engine);
